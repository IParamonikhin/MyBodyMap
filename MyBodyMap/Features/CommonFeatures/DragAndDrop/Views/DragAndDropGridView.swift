...
public struct DragAndDropGridView<Item: Identifiable & Equatable>: View {
    @Bindable var store: StoreOf<DragAndDropFeature<Item>>
    let itemView: (Item, Bool) -> AnyView
    let columns: [GridItem]

    public init(store: StoreOf<DragAndDropFeature<Item>>, columns: [GridItem], @ViewBuilder itemView: @escaping (Item, Bool) -> AnyView) {
        self.store = store
            ...
            ForEach(Array(store.items.enumerated()), id: \.1.id) { idx, item in
                itemView(item, store.draggingID == item.id)
                    ...
                    .dropDestination(for: String.self) { dropItems, _ in
                        guard let draggingID = store.draggingID,
