...
public struct AllDrinksView: View {
    ...
    public var body: some View {
        Color("BGColor")
            ...
            .sheet(
                item: $store.scope(state: \.barcodeScanner, action: \.barcodeScanner)
            ) { scannerStore in
                BarcodeScannerScreen { code in
                    if let code = code {
                        store.send(.barcodeScanner(.presented(.scanned(code))))
                    } else {
                        store.send(.barcodeScanner(.presented(.cancel)))
                    }
                }
            }
    }
